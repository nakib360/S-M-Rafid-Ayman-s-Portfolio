...
const SocialMediaDesign = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        const data = await fetchUploadsByCategory("social");
        setImages(data);
      } catch (fetchError) {
        setError(fetchError.message || "Failed to load social media designs.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="text-white md:px-6 pb-20">
      <div className="max-w-7xl mx-auto text-center mb-16">
        ...
        >
          Scroll-stopping visuals designed to elevate brand presence and engagement.
        </motion.p>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && !error && images.length === 0 && (
          <p className="text-center text-gray-400">
            No social media design uploaded yet.
          </p>
        )}
