...
const LogoDesign = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        const data = await fetchUploadsByCategory("logo");
        setImages(data);
      } catch (fetchError) {
        setError(fetchError.message || "Failed to load logo designs.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="text-white md:px-6 pb-20">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && !error && images.length === 0 && (
          <p className="text-center text-gray-400">No logo design uploaded yet.</p>
        )}
