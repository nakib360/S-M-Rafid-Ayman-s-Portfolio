...
const PrintDesign = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        const data = await fetchUploadsByCategory("print");
        setImages(data);
      } catch (fetchError) {
        setError(fetchError.message || "Failed to load print designs.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="text-white md:px-6 pb-20">
      <div className="max-w-7xl mx-auto text-center mb-16">
        ...
        >
          Clean and professional print materials that communicate clearly.
        </motion.p>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && !error && images.length === 0 && (
          <p className="text-center text-gray-400">No print design uploaded yet.</p>
        )}
